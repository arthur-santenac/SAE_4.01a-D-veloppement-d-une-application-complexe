...
async function afficherDetailsAeroport(aeroportId) {
    try {
        // Récupérer les détails de l'aéroport
...
function afficherFormulaireAeroport(aeroport) {
    ...
    divDroite.innerHTML = `
        ...
        <div class="form-group">
            <label>ID Aéroport</label>
            <input type="text" id="idAeroport" value="${aeroport ? aeroport.idAeroport : ''}" disabled />
        </div>
        <div class="form-group">
    ...
    // Ajouter les listeners après création du DOM
    ...
    document.getElementById('btn-annuler').addEventListener('click', annulerAeroport);
}
...
async function sauvegarderAeroport() {
    ...
    try {
        if (aeroportActuelId) {
            // Modifier un aéroport existant
            await fetch(`${API_URL_aeroport}/${aeroportActuelId}`, {
                method: 'PUT',
